...
如果core=5，max=10。线程池中的10个任务同时执行完上一个任务，开始getTask（），这是，10个线程的wc都为10，timed=true，会调用compareAndDecrementWorkerCount，线程池中的worker会被删除：ctl的wc会-1，并且在processWorkerExit方法中移除worker
